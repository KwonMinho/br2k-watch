...
module.exports = class EtcdClient{
    ...
    async addEndMember(){
        try{
            const end = this.curReplicas;
            const result = await this.client.cluster.memberAdd({
                isLearner: true,
                peerURLs: [`http://${this.urls[end]}:2380`]
            })
            return '';
        }catch(e){
            return e
    ...
    async removeEndMember(){
        try{
            const results = await this.client.cluster.memberList({linearizable: true});
            const endIndex = results.members.length;
            const id = this._extractMemberID(results.members, `etcd${endIndex}`);
            await this.client.cluster.memberRemove({ID: id})
            return '';
        }catch(e){
            return e;
